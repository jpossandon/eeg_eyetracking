...
if (cfg.remove_eye || cfg.remove_m) && cfg.raw
    ...
    if ~isempty(cfg.clean_name)
        load([cfg.preprocanalysisfolder 'cleaning/' cfg.sujid '/' cfg.filename cfg.clean_name '.mat'],'bad','badchans');
        ...
        if cfg.raw
             eegplot( EEG.data, 'srate', EEG.srate, 'title', 'Scroll component activities -- eegplot()', ...
                 'limits', [EEG.xmin EEG.xmax]*1000  ,'spacing',20, 'winlength', 20, 'events', EEG.event, ...
             ...
             eegplot( EEG.icaact, 'srate', EEG.srate, 'title', 'Scroll component activities -- eegplot()', ...
                 'limits', [EEG.xmin EEG.xmax]*1500 , 'winlength', 20, 'events', EEG.event, ...
                 'winrej',autobad, 'command', 'TMPREJI    = sort(TMPREJ(:,1:2));', 'eloc_file', EEG.icachansind ); 
        end
    else
                  eegplot( EEG.data, 'srate', EEG.srate, 'title', 'Scroll component activities -- eegplot()', ...
                 'limits', [EEG.xmin EEG.xmax]*1000 , 'winlength', 20, 'events', EEG.event, ...
...
%                  'command', 'TMPREJI    = sort(TMPREJ(:,1));', 'eloc_file', EEG.chanlocs ); 
     ...
     if evalin('base','exist(''TMPREJI'')') % TODO:iamo not sure this works well
        ...
        if strfind(cfg.clean_name,'vischeck')
           save([cfg.preprocanalysisfolder 'cleaning/' cfg.sujid '/' cfg.filename cfg.clean_name],'bad','badchans');
        else
            save([cfg.preprocanalysisfolder 'cleaning/' cfg.sujid '/' cfg.filename cfg.clean_name '_vischeck.mat'],'bad','badchans');
        end
