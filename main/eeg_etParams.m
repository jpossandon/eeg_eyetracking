function cfg = eeg_etParams(varargin)


if ~isstruct(varargin{1})
    % default parameter for experiment EEG features
    cfg.expname             = 'CEM';                         % used for eeg population stats and meta-analysis across experiments, keep the same name for all analysis done with the same experimental data

    % paths
    cfg.expfolder           = '/net/store/nbp/EEG/CEM/';
     cfg.edfreadpath        = '/net/store/users/jossando/edfread/build/linux64/';
   
    % triggers and trial definition
%     cfg.trial_trig_eeg          = {'S  1','S  2','S  3'};                          % 'S  1','S  2','S  3', vm task eeg triggers 
%     cfg.trial_trig_et           = {'1','2','3'};                                % vm task EDF triggers
%     cfg.trial_time              = [0 12000];                                    % experiment trial window
...
%     cfg.trim_percent            = 1;
    ...
    cfg.sujid                   = 'jo';                                        % id of the subject as how is in EdF and EEG files names
...
% cfg.chanloc             = [cfg.expfolder 'locs_new.sph'];
...
% cfg.EDFfolder           = [cfg.datapath upper(cfg.sujid)  '/' cfg.task_id '/EDF/'];
% cfg.xensor              = [cfg.datapath upper(cfg.sujid) '/xensor/'];
% cfg.eegfolder           = [cfg.datapath upper(cfg.sujid)  '/' cfg.task_id '/eeg/'];
% cfg.matfolder           = [cfg.datapath upper(cfg.sujid)  '/' cfg.task_id '/mats/'];
